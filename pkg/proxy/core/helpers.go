...
// DecompressJWTToken decompresses and reconstructs a JWT token
func DecompressJWTToken(compressed []byte) (string, error) {
	...
	if signatureStart <= compressedDataStart {
		return "", errors.New("token structure invalid: signature overlaps data")
	}
