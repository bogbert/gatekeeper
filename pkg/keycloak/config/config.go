...
type Config struct {
	...
	Tags                               map[string]string         `json:"tags,omitempty" usage:"keypairs passed to the templates at render,e.g title=Page" yaml:"tags"`
	DefaultAllowedQueryParams          map[string]string         `json:"default-allowed-query-params,omitempty" usage:"default allowed query params, sent to IDP key=value" yaml:"default-allowed-query-params"`
	AllowedQueryParams                 map[string]string         `json:"allowed-query-params,omitempty" usage:"allowed query params, sent to IDP key=optional value" yaml:"allowed-query-params"`
	ResponseHeaders                    map[string]string         `json:"response-headers,omitempty" usage:"custom headers to added to the http response key=value" yaml:"response-headers"`
	Headers                            map[string]string         `json:"headers,omitempty" usage:"custom headers to the upstream request, key=value" yaml:"headers"`
	OpenIDProviderHeaders              map[string]string         `json:"openid-provider-headers,omitempty" usage:"http headers sent to idp provider" yaml:"openid-provider-headers"`
	DiscoveryURI                       *url.URL                  `json:"-"`
	OpaAuthzURL                        *url.URL                  `json:"-"`
	TLSForwardingCAPrivateKey          string                    `env:"TLS_FORWARDING_CA_PRIVATE_KEY" json:"tls-forwarding-ca-private-key,omitempty" usage:"path to the CA private key, used for generating server cert for forwarding proxy" yaml:"tls-forwarding-ca-private-key"`
	TLSStoreCACertificate              string                    `env:"TLS_STORE_CA_CERTIFICATE" json:"tls-store-ca-certificate,omitempty" usage:"path to the ca certificate used for verifying trusted server certificates" yaml:"tls-store-ca-certificate"`
	PostLoginRedirectPath              string                    `env:"POST_LOGIN_REDIRECT_PATH" json:"post-login-redirect-path,omitempty" usage:"path to which client is redirected after successful login, in case user access /" yaml:"post-login-redirect-path"`
	RevocationEndpoint                 string                    `env:"REVOCATION_URL" json:"revocation-url,omitempty" usage:"url for the revocation endpoint to revoke refresh token" yaml:"revocation-url"`
	OpenIDProviderProxy                string                    `env:"OPENID_PROVIDER_PROXY" json:"openid-provider-proxy,omitempty" usage:"proxy for communication with the openid provider" yaml:"openid-provider-proxy"`
	UpstreamProxy                      string                    `env:"UPSTREAM_PROXY" json:"upstream-proxy,omitempty" usage:"proxy for communication with upstream" yaml:"upstream-proxy"`
	UpstreamNoProxy                    string                    `env:"UPSTREAM_NO_PROXY" json:"upstream-no-proxy,omitempty" usage:"list of upstream destinations which should be not proxied" yaml:"upstream-no-proxy"`
	BaseURI                            string                    `env:"BASE_URI" json:"base-uri,omitempty" usage:"common prefix for all URIs" yaml:"base-uri"`
	OAuthURI                           string                    `env:"OAUTH_URI" json:"oauth-uri,omitempty" usage:"the uri for proxy oauth endpoints" yaml:"oauth-uri"`
	Upstream                           string                    `env:"UPSTREAM_URL" json:"upstream-url,omitempty" usage:"url for the upstream endpoint you wish to proxy" yaml:"upstream-url"`
	UpstreamCA                         string                    `env:"UPSTREAM_CA" json:"upstream-ca,omitempty" usage:"the path to a file container a CA certificate to validate the upstream tls endpoint" yaml:"upstream-ca"`
	RequestIDHeader                    string                    `env:"REQUEST_ID_HEADER" json:"request-id-header,omitempty" usage:"the http header name for request id" yaml:"request-id-header"`
	ContentSecurityPolicy              string                    `env:"CONTENT_SECURITY_POLICY" json:"content-security-policy,omitempty" usage:"specify the content security policy" yaml:"content-security-policy"`
	OpaAuthzURI                        string                    `env:"OPA_AUTHZ_URI"            json:"opa-authz-uri,omitempty"            usage:"OPA endpoint address with path"                                                                      yaml:"opa-authz-uri"`
	CookieDomain                       string                    `env:"COOKIE_DOMAIN" json:"cookie-domain,omitempty" usage:"domain the access cookie is available to, defaults host header" yaml:"cookie-domain"`
	CookiePath                         string                    `env:"COOKIE_PATH" json:"cookie-path,omitempty" usage:"path for which cookie is valid" yaml:"cookie-path"`
	CookieAccessName                   string                    `env:"COOKIE_ACCESS_NAME" json:"cookie-access-name,omitempty" usage:"name of the cookie used to hold the access token" yaml:"cookie-access-name"`
	CookieIDTokenName                  string                    `env:"COOKIE_ID_TOKEN_NAME" json:"cookie-id-token-name,omitempty" usage:"name of the cookie used to hold id token" yaml:"cookie-id-token-name"`
	CookieRefreshName                  string                    `env:"COOKIE_REFRESH_NAME" json:"cookie-refresh-name,omitempty" usage:"name of the cookie used to hold the encrypted refresh token" yaml:"cookie-refresh-name"`
	CookieOAuthStateName               string                    `env:"COOKIE_OAUTH_STATE_NAME" json:"cookie-oauth-state-name,omitempty" usage:"name of the cookie used to hold the Oauth request state" yaml:"cookie-oauth-state-name"`
	CookieRequestURIName               string                    `env:"COOKIE_REQUEST_URI_NAME" json:"cookie-request-uri-name,omitempty" usage:"name of the cookie used to hold the request uri" yaml:"cookie-request-uri-name"`
	CookiePKCEName                     string                    `env:"COOKIE_PKCE_NAME" json:"cookie-pkce-name,omitempty" usage:"name of the cookie used to hold PKCE code verifier" yaml:"cookie-pkce-name"`
	CookieUMAName                      string                    `env:"COOKIE_UMA_NAME" json:"cookie-uma-name,omitempty" usage:"name of the cookie used to hold the UMA RPT token" yaml:"cookie-uma-name"`
	SameSiteCookie                     string                    `env:"SAME_SITE_COOKIE" json:"same-site-cookie,omitempty" usage:"enforces cookies to be send only to same site requests according to the policy (can be Strict|Lax|None)" yaml:"same-site-cookie"`
	TLSCertificate                     string                    `env:"TLS_CERTIFICATE" json:"tls-cert,omitempty" usage:"path to the TLS certificate" yaml:"tls-cert"`
	TLSPrivateKey                      string                    `env:"TLS_PRIVATE_KEY" json:"tls-private-key,omitempty" usage:"path to the private key for TLS" yaml:"tls-private-key"`
	TLSClientCertificate               string                    `env:"TLS_CLIENT_CERTIFICATE" json:"tls-client-certificate,omitempty" usage:"path to the client certificate used for authenticating to upstream" yaml:"tls-client-certificate"`
	TLSClientPrivateKey                string                    `env:"TLS_CLIENT_PRIVATE_KEY" json:"tls-client-private-key,omitempty" usage:"path to the client private key, used for authentication to upstream" yaml:"tls-client-private-key"`
	TLSClientCACertificate             string                    `env:"TLS_CLIENT_CA_CERTIFICATE" json:"tls-client-ca-certificate,omitempty" usage:"path to the client CA certificate, used for verifying client certificates" yaml:"tls-client-ca-certificate"`
	TLSForwardingCACertificate         string                    `env:"TLS_FORWARDING_CA_CERTIFICATE" json:"tls-forwarding-ca-certificate,omitempty" usage:"path to the CA certificate, used for generating server cert for forwarding-proxy" yaml:"tls-forwarding-ca-certificate"`
	PostLogoutRedirectURI              string                    `env:"POST_LOGOUT_REDIRECT_URI" json:"post-logout-redirect-uri,omitempty" usage:"url to which client is redirected after successful logout" yaml:"post-logout-redirect-uri"`
	TLSMinVersion                      string                    `env:"TLS_MIN_VERSION" json:"tls-min-version,omitempty" usage:"specify server minimal TLS version one of tlsv1.2,tlsv1.3" yaml:"tls-min-version"`
	TLSAdminCertificate                string                    `env:"TLS_ADMIN_CERTIFICATE" json:"tls-admin-cert,omitempty" usage:"path to the TLS certificate" yaml:"tls-admin-cert"`
	TLSAdminPrivateKey                 string                    `env:"TLS_ADMIN_PRIVATE_KEY" json:"tls-admin-private-key,omitempty" usage:"path to the private key for TLS" yaml:"tls-admin-private-key"`
	TLSAdminClientCACertificate        string                    `env:"TLS_ADMIN_CLIENT_CA_CERTIFICATE" json:"tls-admin-client-ca-certificate,omitempty" usage:"path to the client CA certificate user for verifying client certificates" yaml:"tls-admin-client-certificate"`
	ConfigFile                         string                    `env:"CONFIG_FILE" json:"config,omitempty" usage:"path the a configuration file" yaml:"config"`
	TLSStoreClientCertificate          string                    `env:"TLS_STORE_CLIENT_CERTIFICATE" json:"tls-store-client-certificate,omitempty" usage:"path to the client certificate, used for authenticating to store" yaml:"tls-store-client-certificate"`
	TLSStoreClientPrivateKey           string                    `env:"TLS_STORE_CLIENT_PRIVATE_KEY" json:"tls-store-client-private-key,omitempty" usage:"path to the client private key, used for authenticating to store" yaml:"tls-store-client-private-key"`
	StoreURL                           string                    `env:"STORE_URL" json:"store-url,omitempty" usage:"url for the storage subsystem, e.g redis://user:secret@localhost:6379/0?protocol=3, only supported is redis usig redis uri spec" yaml:"store-url"`
	EncryptionKey                      string                    `env:"ENCRYPTION_KEY" json:"encryption-key,omitempty" usage:"encryption key used to encryption the session state" yaml:"encryption-key"`
	LetsEncryptCacheDir                string                    `env:"LETS_ENCRYPT_CACHE_DIR" json:"letsencrypt-cache-dir,omitempty" usage:"path where cached letsencrypt certificates are stored" yaml:"letsencrypt-cache-dir"`
	SignInPage                         string                    `env:"SIGN_IN_PAGE" json:"sign-in-page,omitempty" usage:"path to custom template displayed for signin" yaml:"sign-in-page"`
	RegisterPage                       string                    `env:"REGISTER_PAGE" json:"register-page,omitempty" usage:"path to custom template displayed for registration" yaml:"register-page"`
	ForbiddenPage                      string                    `env:"FORBIDDEN_PAGE" json:"forbidden-page,omitempty" usage:"path to custom template used for access forbidden" yaml:"forbidden-page"`
	ErrorPage                          string                    `env:"ERROR_PAGE" json:"error-page,omitempty" usage:"path to custom template displayed for http.StatusBadRequest" yaml:"error-page"`
	ForwardingGrantType                string                    `env:"FORWARDING_GRANT_TYPE" json:"forwarding-grant-type,omitempty" usage:"grant-type to use when logging into the openid provider, can be one of password, client_credentials" yaml:"forwarding-grant-type"`
	ForwardingUsername                 string                    `env:"FORWARDING_USERNAME" json:"forwarding-username,omitempty" usage:"username to use when logging into the openid provider" yaml:"forwarding-username"`
	ForwardingPassword                 string                    `env:"FORWARDING_PASSWORD" json:"forwarding-password,omitempty" usage:"password to use when logging into the openid provider" yaml:"forwarding-password"`
	Realm                              string                    `json:"-"`
	TLSOpenIDProviderCACertificate     string                    `env:"TLS_OPENID_PROVIDER_CA_CERTIFICATE" json:"tls-openid-provider-ca-certificate,omitempty" usage:"path to the ca certificate for IDP" yaml:"tls-openid-provider-ca-certificate"`
	TLSOpenIDProviderClientPrivateKey  string                    `env:"TLS_OPENID_PROVIDER_CLIENT_PRIVATE_KEY" json:"tls-openid-provider-client-private-key,omitempty" usage:"path to the client private key for IDP" yaml:"tls-openid-provider-client-private-key"`
	TLSOpenIDProviderClientCertificate string                    `env:"TLS_OPENID_PROVIDER_CLIENT_CERTIFICATE" json:"tls-openid-provider-client-certificate,omitempty" usage:"path to the client certificate for IDP" yaml:"tls-openid-provider-client-certificate"`
	Listen                             string                    `env:"LISTEN" json:"listen,omitempty" usage:"Defines the binding interface for main listener, e.g. {address}:{port}. This is required and there is no default value" yaml:"listen"`
	ListenHTTP                         string                    `env:"LISTEN_HTTP" json:"listen-http,omitempty" usage:"interface we should be listening to for HTTP traffic" yaml:"listen-http"`
	ListenAdmin                        string                    `env:"LISTEN_ADMIN" json:"listen-admin,omitempty" usage:"defines the interface to bind admin-only endpoint (live-status, debug, prometheus...). If not defined, this defaults to the main listener defined by Listen" yaml:"listen-admin"`
	ListenAdminScheme                  string                    `env:"LISTEN_ADMIN_SCHEME" json:"listen-admin-scheme,omitempty" usage:"scheme to serve admin-only endpoint (http or https)." yaml:"listen-admin-scheme"`
	DiscoveryURL                       string                    `env:"DISCOVERY_URL" json:"discovery-url,omitempty" usage:"discovery url to retrieve the openid configuration" yaml:"discovery-url"`
	...
	CorsHeaders                        []string                  `json:"cors-headers,omitempty" usage:"set of headers to add to the CORS access control (Access-Control-Allow-Headers)" yaml:"cors-headers"`
	...
	UpstreamKeepaliveTimeout           time.Duration             `env:"UPSTREAM_KEEPALIVE_TIMEOUT" json:"upstream-keepalive-timeout,omitempty" usage:"specifies the keep-alive period for an active network connection" yaml:"upstream-keepalive-timeout"`
	UpstreamTLSHandshakeTimeout        time.Duration             `env:"UPSTREAM_TLS_HANDSHAKE_TIMEOUT" json:"upstream-tls-handshake-timeout,omitempty" usage:"the timeout placed on the tls handshake for upstream" yaml:"upstream-tls-handshake-timeout"`
	UpstreamResponseHeaderTimeout      time.Duration             `env:"UPSTREAM_RESPONSE_HEADER_TIMEOUT" json:"upstream-response-header-timeout,omitempty" usage:"the timeout placed on the response header for upstream" yaml:"upstream-response-header-timeout"`
	UpstreamExpectContinueTimeout      time.Duration             `env:"UPSTREAM_EXPECT_CONTINUE_TIMEOUT" json:"upstream-expect-continue-timeout,omitempty" usage:"the timeout placed on the expect continue for upstream" yaml:"upstream-expect-continue-timeout"`
	MaxIdleConns                       int                       `env:"MAX_IDLE_CONNS" json:"max-idle-connections,omitempty" usage:"max idle upstream / keycloak connections to keep alive, ready for reuse" yaml:"max-idle-connections"`
	MaxIdleConnsPerHost                int                       `env:"MAX_IDLE_CONNS_PER_HOST" json:"max-idle-connections-per-host,omitempty" usage:"limits the number of idle connections maintained per host" yaml:"max-idle-connections-per-host"`
	ServerGraceTimeout                 time.Duration             `env:"SERVER_GRACE_TIMEOUT" json:"server-grace-timeout,omitempty" usage:"the server wait before closing the server" yaml:"server-grace-timeout"`
	ServerReadTimeout                  time.Duration             `env:"SERVER_READ_TIMEOUT" json:"server-read-timeout,omitempty" usage:"the server read timeout on the http server" yaml:"server-read-timeout"`
	...
	EnableProfiling                    bool                      `env:"ENABLE_PROFILING" json:"enable-profiling,omitempty" usage:"switching on the golang profiling via pprof on /debug/pprof, /debug/pprof/heap etc" yaml:"enable-profiling"`
	EnableIDPSessionCheck              bool                      `env:"ENABLE_IDP_SESSION_CHECK" json:"enable-idp-session-check,omitempty" usage:"during token validation it also checks if user session is still present, useful for multiapp logout" yaml:"enable-idp-session-check"`
	EnabledSelfSignedTLS               bool                      `env:"ENABLE_SELF_SIGNED_TLS" json:"enable-self-signed-tls,omitempty" usage:"create self signed certificates for the proxy" yaml:"enable-self-signed-tls"`
	EnableRequestID                    bool                      `env:"ENABLE_REQUEST_ID" json:"enable-request-id,omitempty" usage:"indicates we should add a request id if none found" yaml:"enable-request-id"`
	EnableLogoutRedirect               bool                      `env:"ENABLE_LOGOUT_REDIRECT" json:"enable-logout-redirect,omitempty" usage:"indicates we should redirect to the identity provider for logging out" yaml:"enable-logout-redirect"`
	EnableDefaultDeny                  bool                      `env:"ENABLE_DEFAULT_DENY" json:"enable-default-deny,omitempty" usage:"enables a default denial on all unauthenticated requests, you have to explicitly say what is permitted, although be aware that it allows any valid token" yaml:"enable-default-deny"`
	EnableDefaultDenyStrict            bool                      `env:"ENABLE_DEFAULT_DENY_STRICT" json:"enable-default-deny-strict,omitempty" usage:"enables a default denial on all requests, even valid token is denied unless you create some resources" yaml:"enable-default-deny-strict"`
	EnableEncryptedToken               bool                      `env:"ENABLE_ENCRYPTED_TOKEN" json:"enable-encrypted-token,omitempty" usage:"enable encryption for the access tokens" yaml:"enable-encrypted-token"`
	ForceEncryptedCookie               bool                      `env:"FORCE_ENCRYPTED_COOKIE" json:"force-encrypted-cookie,omitempty" usage:"force encryption for the access tokens in cookies" yaml:"force-encrypted-cookie"`
	EnableLogging                      bool                      `env:"ENABLE_LOGGING" json:"enable-logging,omitempty" usage:"enable http logging of the requests" yaml:"enable-logging"`
	EnableJSONLogging                  bool                      `env:"ENABLE_JSON_LOGGING" json:"enable-json-logging,omitempty" usage:"switch on json logging rather than text" yaml:"enable-json-logging"`
	EnableForwarding                   bool                      `env:"ENABLE_FORWARDING" json:"enable-forwarding,omitempty" usage:"enables the forwarding proxy mode, signing outbound request" yaml:"enable-forwarding"`
	EnableSecurityFilter               bool                      `env:"ENABLE_SECURITY_FILTER" json:"enable-security-filter,omitempty" usage:"enables the security filter handler" yaml:"enable-security-filter"`
	EnableRefreshTokens                bool                      `env:"ENABLE_REFRESH_TOKEN" json:"enable-refresh-tokens,omitempty" usage:"enables the handling of the refresh tokens" yaml:"enable-refresh-tokens"`
	EnableSessionCookies               bool                      `env:"ENABLE_SESSION_COOKIES" json:"enable-session-cookies,omitempty" usage:"access and refresh tokens are session only i.e. removed browser close" yaml:"enable-session-cookies"`
	EnableLoginHandler                 bool                      `env:"ENABLE_LOGIN_HANDLER" json:"enable-login-handler,omitempty" usage:"enables the handling of the refresh tokens" yaml:"enable-login-handler"`
	EnableRegisterHandler              bool                      `env:"ENABLE_REGISTER_HANDLER" json:"enable-register-handler,omitempty" usage:"enables the register handler" yaml:"enable-register-handler"`
	EnableTokenHeader                  bool                      `env:"ENABLE_TOKEN_HEADER" json:"enable-token-header,omitempty" usage:"enables the token authentication header X-Auth-Token to upstream" yaml:"enable-token-header"`
	EnableAuthorizationHeader          bool                      `env:"ENABLE_AUTHORIZATION_HEADER" json:"enable-authorization-header,omitempty" usage:"adds the authorization header to the proxy request" yaml:"enable-authorization-header"`
	EnableHeaderEncoding               bool                      `env:"ENABLE_HEADER_ENCODING" json:"enable-header-encoding,omitempty" usage:"encodes header values according RFC 2047, in MIME B format" yaml:"enable-header-encoding"`
	EnableAuthorizationCookies         bool                      `env:"ENABLE_AUTHORIZATION_COOKIES" json:"enable-authorization-cookies,omitempty" usage:"adds the authorization cookies to the uptream proxy request" yaml:"enable-authorization-cookies"`
	EnableHTTPSRedirect                bool                      `env:"ENABLE_HTTPS_REDIRECT" json:"enable-https-redirection,omitempty" usage:"enable the http to https redirection on the http service" yaml:"enable-https-redirection"`
	SkipOpenIDProviderTLSVerify        bool                      `env:"SKIP_OPENID_PROVIDER_TLSVERIFY" json:"skip-openid-provider-tls-verify,omitempty" usage:"skip the verification of any TLS communication with the openid provider" yaml:"skip-openid-provider-tls-verify"`
	EnableMetrics                      bool                      `env:"ENABLE_METRICS" json:"enable-metrics,omitempty" usage:"enable the prometheus metrics collector on /oauth/metrics" yaml:"enable-metrics"`
	EnableBrowserXSSFilter             bool                      `env:"ENABLE_BROWSER_XSS_FILTER" json:"filter-browser-xss,omitempty" usage:"enable the adds the X-XSS-Protection header with mode=block" yaml:"filter-browser-xss"`
	EnableContentNoSniff               bool                      `env:"ENABLE_CONTENT_NO_SNIFF" json:"filter-content-nosniff,omitempty" usage:"adds the X-Content-Type-Options header with the value nosniff" yaml:"filter-content-nosniff"`
	EnableFrameDeny                    bool                      `env:"ENABLE_FRAME_DENY" json:"filter-frame-deny,omitempty" usage:"enable to the frame deny header" yaml:"filter-frame-deny"`
	LocalhostMetrics                   bool                      `env:"LOCALHOST_METRICS" json:"localhost-metrics,omitempty" usage:"enforces the metrics page can only been requested from 127.0.0.1" yaml:"localhost-metrics"`
	EnableCompression                  bool                      `env:"ENABLE_COMPRESSION" json:"enable-compression,omitempty" usage:"enable gzip compression for response" yaml:"enable-compression"`
	EnableCompressToken                bool                      `env:"ENABLE_COMPRESS_TOKEN" json:"enable-compress-token,omitempty" usage:"enable token gzip compression" yaml:"enable-compress-token"`
	EnablePKCE                         bool                      `env:"ENABLE_PKCE"              json:"enable-pkce,omitempty"              usage:"enable pkce for auth code flow, only S256 code challenge supported"                                  yaml:"enable-pkce"`
	PreserveHost                       bool                      `env:"PRESERVE_HOST" json:"preserve-host,omitempty" usage:"preserve the host header of the proxied request in the upstream request" yaml:"preserve-host"`
	EnableUma                          bool                      `env:"ENABLE_UMA"               json:"enable-uma,omitempty"               usage:"enable uma authorization, please don't use it in production, we would like to receive feedback"      yaml:"enable-uma"`
	EnableOpa                          bool                      `env:"ENABLE_OPA"               json:"enable-opa,omitempty"               usage:"enable authorization with external Open policy agent"                                                yaml:"enable-opa"`
	SecureCookie                       bool                      `env:"SECURE_COOKIE" json:"secure-cookie,omitempty" usage:"enforces the cookie to be secure" yaml:"secure-cookie"`
	HTTPOnlyCookie                     bool                      `env:"HTTP_ONLY_COOKIE" json:"http-only-cookie,omitempty" usage:"enforces the cookie is in http only mode" yaml:"http-only-cookie"`
	EnableIDTokenCookie                bool                      `env:"ENABLE_IDTOKEN_COOKIE" json:"enable-id-token-cookie,omitempty" usage:"enable id token cookie" yaml:"enable-id-token-cookie"`
	EnableUmaMethodScope               bool                      `env:"ENABLE_UMA_METHOD_SCOPE" json:"enable-uma-method-scope,omitempty" usage:"enables passing request method as 'method:GET' scope to keycloak for authorization" yaml:"enable-uma-method-scope"`
	SkipUpstreamTLSVerify              bool                      `env:"SKIP_UPSTREAM_TLS_VERIFY" json:"skip-upstream-tls-verify,omitempty" usage:"skip the verification of any upstream TLS" yaml:"skip-upstream-tls-verify"`
	CorsCredentials                    bool                      `env:"CORS_CREDENTIALS" json:"cors-credentials,omitempty" usage:"credentials access control header (Access-Control-Allow-Credentials)" yaml:"cors-credentials"`
	EnableHmac                         bool                      `env:"ENABLE_HMAC" json:"enable-hmac,omitempty" usage:"enable creating hmac for forwarded requests and verification on incoming requests" yaml:"enable-hmac"`
	NoProxy                            bool                      `env:"NO_PROXY" json:"no-proxy,omitempty" usage:"do not proxy requests to upstream, useful for forward-auth usage (with nginx, traefik)" yaml:"no-proxy"`
	NoRedirects                        bool                      `env:"NO_REDIRECTS" json:"no-redirects,omitempty" usage:"do not have back redirects when no authentication is present, 401 them" yaml:"no-redirects"`
	SkipAccessTokenIssuerCheck         bool                      `env:"SKIP_ACCESS_TOKEN_ISSUER_CHECK" json:"skip-access-token-issuer-check,omitempty" usage:"according RFC issuer should not be checked on access token, this will be default true in future" yaml:"skip-access-token-issuer-check"`
	SkipAccessTokenClientIDCheck       bool                      `env:"SKIP_ACCESS_TOKEN_CLIENT_ID_CHECK" json:"skip-access-token-clientid-check,omitempty" usage:"according RFC client id should not be checked on access token, this will be default true in future" yaml:"skip-access-token-clientid-check"`
	SkipAuthorizationHeaderIdentity    bool                      `env:"SKIP_AUTHORIZATION_HEADER_IDENTITY" json:"skip-authorization-header-identity,omitempty" usage:"skip authorization header identity, means that we won't be extracting token from authorization header (e.g. if authorization header is used only by application behind gatekeeper)" yaml:"skip-authorization-header-identity"`
	UpstreamKeepalives                 bool                      `env:"UPSTREAM_KEEPALIVES" json:"upstream-keepalives,omitempty" usage:"enables or disables the keepalive connections for upstream endpoint" yaml:"upstream-keepalives"`
	Verbose                            bool                      `env:"VERBOSE" json:"verbose,omitempty" usage:"switch on debug / verbose logging" yaml:"verbose"`
	EnableProxyProtocol                bool                      `env:"ENABLE_PROXY_PROTOCOL" json:"enabled-proxy-protocol,omitempty" usage:"enable proxy protocol" yaml:"enabled-proxy-protocol"`
	UseLetsEncrypt                     bool                      `env:"USE_LETS_ENCRYPT" json:"use-letsencrypt,omitempty" usage:"use letsencrypt for certificates" yaml:"use-letsencrypt"`
	DisableAllLogging                  bool                      `env:"DISABLE_ALL_LOGGING" json:"disable-all-logging,omitempty" usage:"disables all logging to stdout and stderr" yaml:"disable-all-logging"`
	EnableLoA                          bool                      `env:"ENABLE_LOA" json:"enable-loa,omitempty" usage:"enables level of authentication" yaml:"enable-loa"`
	EnableStoreHA                      bool                      `env:"ENABLE_STORE_HA" json:"enable-store-ha,omitempty" usage:"enable store high availability client, currently only redis-cluster supported" yaml:"enable-store-ha"`
	EnableSigning                      bool                      `env:"ENABLE_SIGNING" json:"enable-signing,omitempty" usage:"enable signing of requests to upstream, when in reverse proxy mode" yaml:"enable-signing"`
	EnableSigningHmac                  bool                      `env:"ENABLE_SIGNING_HMAC" json:"enable-signing-hmac,omitempty" usage:"enable signing of requests to upstream, when in reverse proxy mode with HMAC" yaml:"enable-signing-hmac"`
	EnableXForwardedHeaders            bool                      `env:"ENABLE_X_FORWARDED_HEADERS" json:"enable-x-forwarded-headers,omitempty" usage:"enable using X-Forwarded headers (host and proto) for callback and authorization url" yaml:"enable-x-forwarded-headers"`
	EnableOptionalEncryption           bool                      `env:"ENABLE_OPTIONAL_ENCRYPTION" json:"enable-optional-encryption,omitempty" usage:"enable optional decryption for access tokens, id tokens, refresh tokens" yaml:"enable-optional-encryption"`
	EnableLogoutAuth                   bool                      `env:"ENABLE_LOGOUT_AUTH" json:"enable-logout-auth,omitempty" usage:"enable authentication on logout handler" yaml:"enable-logout-auth"`
	EnableRequestUpstreamCompression   bool                      `env:"ENABLE_REQUEST_UPSTREAM_COMPRESSION" json:"enable-request-upstream-compression,omitempty" usage:"enables asking upstream for compression, by adding Accept-Encoding: gzip header and decompressing response from upstream" yaml:"enable-request-upstream-compression"`
	...
	ExtIDPUsersFileReloadInterval      time.Duration             `env:"EXTIDP_USERS_FILE_RELOAD_INTERVAL" json:"extidp-users-file-reload-interval,omitempty" usage:"interval to check for users file updates (in seconds)" yaml:"extidp-users-file-reload-interval"`
...
func (r *Config) IsValid(fileCheckEnable bool) error {
	...
	err := r.isAdminTLSFilesValid(fileCheckEnable)
	if err != nil {
		return err
	}

	err = r.isTLSFilesValid(fileCheckEnable)
	if err != nil {
		return err
	}
...
//nolint:cyclop
func (r *Config) isTLSFilesValid(fileCheckEnable bool) error {
...
//nolint:cyclop
func (r *Config) isAdminTLSFilesValid(fileCheckEnable bool) error {
